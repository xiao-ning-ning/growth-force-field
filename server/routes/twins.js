...
// POST /api/twins/create - 从用户数据创建数字孪生
router.post('/create', async (req, res) => {
  try {
    ...
    if (req.session.user?.role !== 'admin') {
      return res.status(403).json({ error: '仅管理员可创建数字孪生' });
    }
...
// DELETE /api/twins/:id - 删除数字孪生
router.delete('/:id', (req, res) => {
  try {
    if (req.session.user?.role !== 'admin') {
      return res.status(403).json({ error: '仅管理员可删除数字孪生' });
    }
    const twins = loadTwins();
...
// PUT /api/twins/:id - 更新数字孪生人格参数
router.put('/:id', (req, res) => {
  try {
    if (req.session.user?.role !== 'admin') {
      return res.status(403).json({ error: '仅管理员可编辑数字孪生' });
    }
    const twins = loadTwins();
    const idx = twins.findIndex(t => t.id === req.params.id);
    if (idx === -1) return res.status(404).json({ error: '分身不存在' });
